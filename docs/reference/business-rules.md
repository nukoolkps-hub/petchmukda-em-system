...
## ราคาทอง 96.5% (สมาคมค้าทองคำ) + สูตรในความรู้ต่างๆ

ระบบ "ความรู้ต่างๆ" (`/knowledge`) ใช้ราคาทองสมาคม 96.5% สด ที่ดึงอัตโนมัติทุก 15 นาที (Cloud Function `fetchGoldPriceScheduled` · `/config/goldPrice`) → live tables + calculator default + live-example คำนวณตามราคาวันนี้
...
| 30% | × 0.3 | ค่าเปลี่ยนนาก ปกติ (ไม่ตรวจ %) |
...
ค่าจาก `CHANGE_PRICE_WEIGHTS` ใน `src/utils/changePriceUtils.ts` — single source of truth

| น้ำหนัก | id | grams | ค่าแรงเริ่มต้น |
|---|---|---|---|
...
ใช้ **shortcut** สำหรับ ½/1/2 สลึง และ 1 บาท (สูตรร้านจริง) · นอกนั้นใช้ precise grams

`computeSellPrice96` ใน `src/utils/changePriceUtils.ts`:

```ts
// ½ สลึง = ราคาทอง ÷ 8 + ค่าแรง
// 1 สลึง = ราคาทอง ÷ 4 + ค่าแรง
// 2 สลึง = ราคาทอง ÷ 2 + ค่าแรง
// 1 บาท  = ราคาทอง + ค่าแรง (ไม่ต้องหาร)
// อื่นๆ  = (ราคาทอง × 0.0656 × grams) + ค่าแรง
// 2 บาท  = (ราคาทอง + 1,050) × 2 (hardcoded ใน SellPrice96Table)
```

### สูตรราคารับซื้อ 96.5%

`computeBuyPrice96`: `(ราคาทอง × (1 − discount%)) × 0.0656 × grams`
...
ราคาขาย/ชิ้น = (ราคา/กรัม + ค่าแรง) × น้ำหนัก(g)
```
...
### สูตรราคารับซื้อทอง 90 / นาก

| | ทั่วไป | มีตรวจ % |
|---|---|---|
| ทอง 90 | `gold × 60% × 0.0656 × grams` | `gold × (realPct − 10)% × 0.0656 × grams` |
| นาก | `gold × 30% × 0.0656 × grams` | `gold × (realPct − 10)% × 0.0656 × grams` |

### สูตรราคาขายนาก

`(ราคาทอง ÷ 2) + ค่าแรง = ราคาขาย`

### สูตรราคาขาย/รับซื้อเงิน

- ขาย: `(ราคาเงิน/กรัม × น้ำหนัก) + ค่าแรง`
- รับซื้อ: `ราคาเงิน/กรัม × น้ำหนัก` (ไม่มีค่าแรง)

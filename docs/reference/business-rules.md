...
|---|---|
| วันข้างหน้า | ✅ ได้ตลอด |
| วันนี้ · ถึง 08:59 | ✅ ได้ (เปลี่ยนใจมาทำงาน ทันร้านเปิด) |
| วันนี้ · ตั้งแต่ 09:00 | ❌ ล็อก |
| เริ่มลาไปแล้ว (รวมใบลาหลายวันที่ยังไม่จบ) | ❌ ล็อก — ยึด 09:00 ของ**วันแรก** |
| **เพิ่งกดยื่นเองไม่เกิน 10 นาที** | ✅ ได้เสมอ แม้เลยเส้นตาย (ดูช่วงผ่อนผัน) |
...
#### ช่วงผ่อนผัน "กดยื่นผิด" — 10 นาทีแรกหลังยื่น

`BUSINESS_RULES.LEAVE_CANCEL_GRACE_MINUTES` (= 10) · แก้เคสพนักงานยื่นลาของวันนี้ตอนบ่าย
แล้วรู้ตัวว่าเลือกผิดวัน/ผิดประเภท — ตามกฎ 09:00 เพียวๆ จะลบเองไม่ได้ทันทีที่กดยื่นเสร็จ
(หน้ายื่นลาเลือกวันได้ตั้งแต่ "วันนี้" เป็นต้นไป · การ **สร้าง** ใบลาไม่มีเส้นตายเวลา)

ปลอดภัยเพราะใบที่เพิ่งสร้างยังไม่เคยเข้ากล่องประกาศเช้า — ลบแล้วไม่มีใครรับรู้ผิด

⚠️ **เกณฑ์เวลาต้องเป็น `createdAtServer` (`serverTimestamp()`) เท่านั้น** —
`createdAt` มาจากนาฬิกาเครื่องผู้ใช้ ถ้าใช้ตัวนั้น client ที่แก้ค่าส่งขึ้นมาเองสามารถ
ตั้งเวลาอนาคตไว้ แล้วเปิดหน้าต่างลบใบลาย้อนหลังทีหลังได้ · `firestore.rules`
บังคับตอน create ว่า `createdAtServer == request.time` (ปลอมไม่ได้) และเป็น field
**optional** เพื่อไม่ให้ client รุ่นเก่าที่ยังไม่รีเฟรชยื่นลาไม่ได้ — ใบที่ไม่มีฟิลด์นี้
(ของเก่า) แค่ไม่ได้ช่วงผ่อนผัน ใช้เส้นตาย 09:00 อย่างเดียว

ใบที่ **admin เพิ่มให้** (`createdByAdmin`) ไม่เข้าข่าย — ไม่ใช่ "พนักงานกดผิดเอง"
...
2. **Server** — `firestore.rules` → `canSelfCancelLeave(startYmd)` + `withinCancelGrace()`
   ตัดสินด้วย **เวลาไทยฝั่ง server** (`bangkokTime().hours() < 9`) — client ปรับนาฬิกา
   เครื่องแล้วเลี่ยงไม่ได้ · ยืนยันด้วย `src/firebase/leaves.emulator.test.ts` ที่ยิง rules
   จริงบน emulator (รวมเคสปลอม `createdAtServer` ต้องถูกปฏิเสธตั้งแต่ตอนสร้าง)
...
**ข้อความที่พนักงานเห็น** มาจาก `leaveCancelHint()` — เขียนวัน-เวลาเจาะจงของใบนั้นเลย
("ยกเลิกใบลานี้เองได้ถึง 08:59 น. ของ จ. 7 ก.ย. 2569") ไม่ใช่กฎลอยๆ ให้ตีความเอง ·
3 สถานะ: `ok` (ยังลบได้) · `grace` (เหลือแต่ช่วงผ่อนผัน) · `locked` (ให้แจ้ง ADMIN)

Source: `src/utils/leaveUtils.ts` · `src/hooks/useLeaveForm.ts` · `src/components/home/RequestTab.tsx` · `src/firebase/leaves.ts` · `firestore.rules`
...
- **จำนวนครั้ง: 3 ครั้ง/เดือน** (`BUSINESS_RULES.ADVANCE_MAX_PER_MONTH`) — นับ pending +
  approved ที่พนักงานยื่นเอง · rejected ไม่นับ (ยื่นใหม่ได้ในเดือนเดียวกัน) ·
  auto-carry (`autoCarryFromMonth` ตั้ง) ไม่นับเป็นครั้ง **แต่ยังกินวงเงิน % ตามจริง**
...
2. **หน้าที่ที่เหลือ** — ส่ง `exclusiveWorkers` เป็น `blockedEmpIds` เข้า
   `computeDutyForDay` → กันออกจากการเลือกทุกชั้น (คนหลัก · คนแทน · `double_up`)

### คนหลักที่ติดหน้าที่ผูกขาด = "มีคนแทน" ไม่ใช่ "เวรเปลี่ยนมือ"

ถ้า `precomputedPrimary` (คนหลักของรอบนี้) อยู่ใน `blockedEmpIds`
`computeDutyForDay` **คงเขาไว้เป็น `primaryEmpId`** แล้วเดินเส้นทางหาคนแทน
เหมือนตอนลา — ไม่ใช่ไปคำนวณคนหลักใหม่จาก pool ที่ตัดเขาออกแล้ว

เหตุผล (เดิมพลาดทั้ง 2 ข้อ · PR #783):

1. **UI ไม่ขึ้น "แทน X"** — คืน `reason: "rotation"` ทำให้ดูเหมือนเป็นเวรของ
   คนแทนเอง ทั้งที่เจ้าตัวแค่ถูกดึงไปทำงานอื่นวันเดียว
2. **rotation เลื่อน** — ตัดเขาออกจาก pool ทำให้ index ของทุกคนขยับ
   (`pool[(idx + anchor) % pool.length]` เปลี่ยนความหมายทั้งแถว) → คนถัดไป
   โดนข้าม และคนหลักตัวจริงไม่เคยถูกบันทึกว่าถึงเวร

`primaryPulledToDuty: true` ติดมากับ assignment เพื่อให้ UI เขียน
**"แทน X (ติดหน้าที่อื่น)"** แทน **"(ลา)"** — ฝั่ง server ติดป้ายให้คนที่ถูกดึงไป
coverage ด้วย (เขาเข้า `effLeaves` เป็น "ลาปลอม" แต่ไม่ได้ลาจริง) ·
`undefined` = ข้อมูลเก่า/ลาจริง → UI คงข้อความเดิม

ถ้าติดผูกขาดแล้ว**ไม่มีใครแทนได้เลย** → ให้เจ้าตัวทำซ้อนเอง (กฎ "ห้ามมีหน้าที่ว่าง"
ด้านล่างชนะกฎผูกขาด) ไม่ใช่คืน `all_on_leave`

### กฎเหนือกว่า: **ห้ามมีหน้าที่ว่าง**

`blockedEmpIds` (คนผูกขาด) และ `excludeForPrimary` (คนหลักรายเดือน) เป็นแค่
*preference* ทั้งคู่ — ก่อนจะคืน `all_on_leave` `computeDutyForDay` มี
**ทางเลือกสุดท้าย**: สแกน `rawPool` (ปลดตัวกรองทั้ง 2 ตัว) หาใครก็ได้ที่
"มาทำงานวันนี้" มาทำแทน (คืน reason `double_up`)
...
สะสมระหว่างไล่หน้าที่ · ทั้ง `double_up` และทางเลือกสุดท้ายใช้ `pickLeastLoaded`
แทนการ "เจอใครก่อนเอาคนนั้น" · เสมอกันใช้ลำดับสแกนเดิม (deterministic) ·
ไม่ส่ง `loadToday` = พฤติกรรมเดิมเป๊ะ

> ตัวอย่าง 3 คน / 7 หน้าที่ (2 ผูกขาด) วันที่คนที่เหลือลา:
> เดิม a=6 · b=1 → ตอนนี้ **a=4 · b=3**

> ทางเลือกสุดท้ายนี้ยังปิดช่องโหว่เดิมด้วย: `excludeForPrimary` (monthly lock)
> เคยทำให้หน้าที่รายสัปดาห์ว่างตอนคนเหลือน้อยแล้วคนนั้นลา — จำลอง 3 คน/7 หน้าที่
> เคยว่าง 175 ช่อง/ปี **ทั้งที่ยังไม่ได้ติ๊กผูกขาดเลย** ตอนนี้เป็น 0
...
|---|---|---|
| **Gold Traders Association (gold · primary)** | `goldtraders.or.th/api/GoldPrices/Latest?readjson=false` | `bL_SellPrice`, `bL_BuyPrice`, `priceChangeFromPrevRow`, `asTime` |
| **DoDev (silver)** | `price.dodev.me/current_price/silver` | `bid_g_price` (ซื้อ/กรัม), `ask_g_price` (ขาย/กรัม รวม VAT 7%), `bid_kg_price`, `ask_kg_price`, `time` |
...
**ยึด "จอราคาร้าน" เป็นหลัก** — จอ (`petchmukda-price-exc`) คือราคาที่ลูกค้าเห็น
หน้าร้าน · `/api/price` ส่ง `changeRates` มาพร้อมราคาทอง → Cloud Function เก็บลง
`/config/goldPrice.changeRates` · `resolveChangePrice()` ใช้ค่านั้นก่อนเสมอ

จอมีค่าแรง + rate% + วิธีปัดเศษเป็นของตัวเอง (admin แก้ได้ในหน้า admin ของจอ)
จึงได้เลข**ไม่เท่า**สูตรฝั่งระบบพนักงาน (เช่นที่ทอง 68,700: 1 บาท จอ = 3,000 ฿ ·
สูตรในระบบ = 3,050 ฿) — ต้องยึดของจอ ไม่งั้นพนักงานบอกลูกค้าคนละราคากับจอ

**Fallback** — `computeChangePriceBreakdown` ปัดทวีคูณ 50 ฿ · ใช้เมื่อจอเรียกไม่ได้ /
ไม่ส่ง `changeRates` / `changeRatesForPrice` ไม่ตรงราคาทองปัจจุบัน (= ค่าค้าง):
```
...
> ตาราง "ค่าแรงเริ่มต้น" (`/config/laborCost`) ที่ admin แก้ในระบบพนักงาน มีผลกับ
> **ราคาขาย 96.5%** และกับ**ค่าเปลี่ยนเฉพาะตอน fallback** — ค่าเปลี่ยนปกติมาจากจอ
> จะแก้ค่าเปลี่ยนจริงต้องแก้ที่หน้า admin ของจอราคาร้าน
...
| เงิน | `silverBuyPerGram × น้ำหนักสินค้า` (live DoDev) | — |

...
|---|---|---|
| ล้างทั้งระบบ | `wipeTestData` | top-level collections: `employees` (+ `months` subcollection) · `leaves` · `advances` · `employeeLoans` · `payrollConfirms` · `poolSnapshots` · `poolAdjustments` · `dutyAssignmentsToday` · `certCounters` · `recentTips` · `dailySummarySent` |
...
- **`config/*` ไม่ถูกแตะ** (goldPrice/secrets/storeCalendar/laborCost ฯลฯ คงอยู่)

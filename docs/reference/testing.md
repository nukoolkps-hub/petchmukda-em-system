...
| `quizSetEdit.test.ts` | แก้ชุดข้อสอบผ่านหน้า admin — `nextQuestionId` ต้องไล่จาก**เลขสูงสุด** ไม่ใช่จำนวนข้อ (ลบข้อกลางแล้วต้องไม่ชน id เดิม) · `validateQuizSet` จับข้อว่าง/id ซ้ำ (รวมข้ามกลุ่ม)/เวลา-เกณฑ์เพี้ยน และคืนทุกปัญหาพร้อมกัน · `duplicateAsDraft` deep copy ไม่กระทบต้นฉบับ |

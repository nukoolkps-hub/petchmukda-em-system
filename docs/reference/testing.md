...
| `quizAttempt.test.ts` | แบบทดสอบ — นาฬิกาอิง `startedAt` (รีเฟรช/ปิดแท็บแล้วไม่ได้เวลาเพิ่ม) · เกณฑ์ผ่านนับ `quiz.main` เท่านั้น · ตรวจไม่ครบ = `passed: null` ไม่ใช่ `false` · รูปร่าง `BASIC_EXAM` (30+6 · id ไม่ซ้ำ ไม่สลับ) |

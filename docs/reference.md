...
| [`reference/troubleshooting.md`](reference/troubleshooting.md) | เจออาการผิดปกติ (กองกลางเพี้ยน, ยอดเปลี่ยน, ราคาทองไม่มา, deploy fail) — runbook วิธีแก้ |
| [`reference/glossary.md`](reference/glossary.md) | ไม่แน่ใจคำไทย (UI) ตรงกับ identifier ไหนใน code |
...
`socialSecurity`, `slipUrl` ฯลฯ) — เปิดให้พนักงานอ่านของคนอื่นทุกคนไม่ได้
แต่การคำนวณกองกลางต้องรู้ **pieces + roleId + poolExclusion + วันลา** ของคนอื่นทั้งกลุ่ม

วิธีแก้: แยก field ที่ไม่อ่อนไหวออกมาเป็น collection ใหม่ `poolSnapshots/{ym}`
(1 doc/เดือน, map `empId → { normalSalePieces, specialSalePieces, buyPieces, poolItemPieces, roleId, poolExclusion, totalLeaveDays, poolThresholdExempt }`)

**`poolItemPieces`** (PR #494) — multi-item map ครอบ custom pool items (e.g. "ขายมือสอง" id "p_xxx")
จำเป็นสำหรับ employee-side pool calc · ถ้าไม่มี field นี้ resolver จะ fallback 0 สำหรับ custom id (peer calc ผิด)

...
export const notifyAdvanceApproved = onCall(async (request) => {
  ...
  const { empLineUserId, empName, amount, reason, month, slipImg, approvedAt, requestId } =
    request.data as AdvanceApprovedData;
  if (!empLineUserId) throw new HttpsError("invalid-argument", "missing empLineUserId");
  ...
  const flex = {
    ...
    contents: {
      ...
      header: {
        ...
        paddingAll: "16px",
        contents: [
          { type: "text", text: "✅ โอนเงินเรียบร้อยแล้ว", color: "#FFF", weight: "bold", size: "lg" },
          { type: "text", text: "เบิกเงินล่วงหน้า · ห้างเพชรทองมุกดา", color: "#D4F0DD", size: "xs", margin: "sm" },
        ],
      },
      body: {
        type: "box",
        ...
        spacing: "md",
        contents: [
          { type: "text", text: `สวัสดี คุณ${empName}`, size: "md", weight: "bold", color: COLOR.text },
          { type: "text", text: "เงินที่คุณขอเบิกได้รับการอนุมัติและโอนเข้าบัญชีเรียบร้อยแล้ว", size: "sm", color: COLOR.textMid, wrap: true },
          {
            type: "box", layout: "vertical", backgroundColor: COLOR.greenLt, cornerRadius: "8px", paddingAll: "12px", margin: "md",
            contents: [
              { type: "text", text: "จำนวนที่โอน", size: "xs", color: COLOR.textMid },
              { type: "text", text: `฿${TH_NUM(amount)}`, size: "xxl", weight: "bold", color: COLOR.green },
            ],
          },
          ...
          { type: "text", text: `⏰ อนุมัติเมื่อ: ${dtStr}`, size: "xs", color: "#B89A72", margin: "md" },
        ],
      },
      footer: {
        type: "box",
        ...
        spacing: "sm",
        contents: [
          { type: "text", text: "กรุณาตรวจสอบยอดในบัญชีของคุณ", size: "xs", color: COLOR.textMid, align: "center" },
        ],
      },
    },
  };
  ...
  await pushLineMessage(config.LINE_CHANNEL_ACCESS_TOKEN, empLineUserId, messages);

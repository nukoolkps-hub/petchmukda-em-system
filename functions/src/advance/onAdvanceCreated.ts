...
export const onAdvanceCreated = onDocumentCreated(
  ...
  async (event) => {
      ...
      contents: {
        ...
        body: {
          ...
          contents: [
            ...
            { type: "text", text: advance.empName as string, weight: "bold", size: "lg", margin: "md" },
            { type: "text", text: `จำนวน: ฿${(advance.amount as number).toLocaleString("th-TH")}`, size: "sm", margin: "sm" },
    ...
    await pushLineMessage(config.LINE_CHANNEL_ACCESS_TOKEN, config.ADMIN_LINE_USER_ID, flexMessage);
  },
);

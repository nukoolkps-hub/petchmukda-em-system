...
| `published` | **แก้เนื้อหาไม่ได้เลย** — `firestore.rules` บล็อก `update` ยกเว้น**เปลี่ยนชื่อ** (`title` อย่างเดียว · `quizTitleOnly()`) · จะแก้โจทย์/เวลา/เกณฑ์ต้อง "ทำสำเนาเป็นชุดใหม่" · **ลบได้เฉพาะชุดที่ยังไม่มีใบสอบอ้างถึง** ผ่าน callable `deleteQuizSet` (ดูด้านล่าง) |
...
**เปลี่ยนชื่อชุด (ปุ่มดินสอข้างชื่อในการ์ด):** ทำได้ทั้งร่างและ**ชุดที่เผยแพร่แล้ว** — ชื่อเป็นแค่ป้ายเรียก ไม่ใช่สิ่งที่ใช้ตรวจ (โจทย์/จำนวนข้อ/เวลา/เกณฑ์ ยังแช่แข็งครบ ผลสอบเก่าไม่เปลี่ยนตาม) · rules ยอมเฉพาะกรณี `affectedKeys().hasOnly(['title'])` เท่านั้น ไม่ให้แอบแก้ field อื่นติดไปด้วย · `sanitizeQuizTitle` ยุบช่องว่างซ้อน (ชื่อถูกใช้เป็นฐานคิดเลขเวอร์ชันตอนทำสำเนา) · **ถ้ารอบสอบที่เปิดค้างใช้ชุดนี้ ต้องแก้ `config/quizRound.quizTitle` ด้วย** (doc นั้นเก็บชื่อเป็น snapshot ตอนเปิดรอบ) ไม่งั้นการ์ดรอบสอบยังโชว์ชื่อเก่า ดูเหมือนเปลี่ยนไม่ติด
...
- **ผู้สอบพิมพ์ชื่อเองก่อนกดเริ่ม** — ใช้จริงคือ ADMIN เปิดเครื่องให้พนักงานทำ `uid` จึงเป็นของ ADMIN ไม่ใช่คนทำ · ชื่อที่พิมพ์ = สิ่งที่โชว์ในประวัติ/หน้าตรวจ · `employeeId` มาจาก `resolveExamineeId` (จับคู่**ตรงตัว**หลัง trim/ยุบช่องว่าง/ไม่สนตัวพิมพ์ · **ชนกันหลายคน = คืนค่าว่าง ไม่เดา** เพราะผูกผิดคนแย่กว่าไม่ผูก)
- **ยกเลิกกลางคัน = stamp `cancelledAt` ไม่ลบ doc** — ประวัติต้องเห็นว่าเคยเริ่มแล้วเลิก · (คนละเรื่องกับปุ่มถังขยะในรายการประวัติที่ ADMIN กด = **ลบ doc ทิ้งจริง กู้คืนไม่ได้** · 2 จังหวะ ลบ → ยืนยันลบ · ลบใบที่ยัง "กำลังทำ" ได้ด้วย เครื่องผู้สอบจะเด้งกลับหน้ากรอกชื่อเอง) · ยกเลิกแล้ว**ไม่ใช่ "หมดเวลา"** (`isExpired` คืน false) คนละเหตุผลกัน เหมารวมแล้ว UI อ่านผิด · `isInProgress` คือตัวที่บอกว่ายังทำอยู่จริง (ยังไม่ส่ง · ยังไม่ยกเลิก · ยังไม่หมดเวลา) · rules ปิดการเขียนของเจ้าของผ่าน `quizStillOpen()`
...
- **ขนาดตัวหนังสือในหน้าแบบทดสอบ/ตั้งค่าข้อสอบ ใช้มาตรฐานเดียวกับ admin panel อื่น** (`text-sm` หลัก · `text-xs` ข้อมูลรอง · `text-base` หัวการ์ด) · **ข้อยกเว้นมีแค่ 2 จุดใน `QuizRunner`** คือ **โจทย์ข้อสอบ** (`text-xl`) กับ **นาฬิกานับถอยหลัง** (`text-xl` mono) และช่องพิมพ์คำตอบ (`text-base` — พิมพ์ยาว 100 นาที) · อย่าขยายส่วนอื่นตามไปด้วย จะหลุดจากระบบทันที
...
| **หน้าที่ประจำ** (`duties` + `dutyAssignments`) | weekly/monthly rotation · `pickPrimary` เลือกคนทำตาม fairness (เคยทำน้อยสุดได้ก่อน) · **คนแทน (primary ลา):** monthly = fair "เคยแทนน้อยสุดก่อน" (`pickRotationSubstitute` + `replayRotationSubHistory`) · weekly = neighbor-scan · `duty.substituteExcludedEmpIds` = คนที่หมุนเป็นเวรหลักได้แต่ไม่ให้เป็นคนแทน · **`duty.exclusive`** ("ผูกขาดคนทำ" เช่น ONLINE/บัญชี) = คนที่ทำหน้าที่นี้วันนั้น (คนหลัก**หรือคนแทน**) ไม่ถูกจัดหน้าที่อื่นเลย — คำนวณหน้าที่ผูกขาดก่อนแล้วส่ง `blockedEmpIds` เข้าหน้าที่ที่เหลือ · **คนหลักที่ติดผูกขาด = "มีคนแทน" ไม่ใช่ "เวรเปลี่ยนมือ"** — คง `primaryEmpId` ไว้แล้วหาคนแทน (ห้ามคำนวณคนหลักใหม่จาก pool ที่ตัดเขาออก · rotation จะเลื่อนทั้งแถว + UI ไม่ขึ้น "แทน X") · `primaryPulledToDuty` → UI เขียน "แทน X (ติดหน้าที่อื่น)" แทน "(ลา)" · หน้าที่ `kind="coverage"` **ผูกขาดเสมอ ไม่มี toggle** (คนที่ถูกดึงไปแทนเข้า `effLeaves`) · `fillEmptyWithPulled()` ดึงเขากลับมาถ้าไม่งั้นหน้าที่จะว่าง · **ห้ามมีหน้าที่ว่าง** — ก่อนคืน `all_on_leave` มีทางเลือกสุดท้ายสแกน `rawPool` (ปลดทั้ง blockedEmpIds + monthly lock) หาคนที่มาทำงานวันนี้มาทำแทน · `coveragePay` จ่ายแทนเมื่อ target ลา (โชว์สดใน preview ก่อนยืนยันยอดผ่าน snapshot `coverageThisMonth`) · **snapshot** `dutyAssignmentsToday` = assignments วันนี้ + `coverageForecast[]` (คนแทนล่วงหน้าถึงสิ้นปี) + `coverageThisMonth` (เงินค่าแทนสดต่อคน) · `applicableDuties` filter ด้วย storeCalendar ก่อน render · client/server sync ผ่าน `scripts/check-duty-sync.mjs` |
...
| **หนังสือรับรองเงินเดือน** | พนักงานสั่งพิมพ์เองได้ใน /salary · `certCounters` (running counter ต่อปี) · ใช้ `getEffectiveBaseSalary` (เลขปัจจุบัน · ไม่ใช่เริ่มต้น) · **ระบุยอดเองได้** ใน modal — ต่ำกว่าได้เสมอ · สูงกว่าได้ถึง **base × (1 + `CERT_MAX_OVER_BASE`)** = +30% (ค่าคอมกองกลาง/โบนัสไม่อยู่ใน `baseSalary` → ยอดพื้นฐานล้วนต่ำกว่ารายได้จริง) · กฎอยู่ที่ `src/utils/certSalary.ts` **ที่เดียว** เรียกจาก 3 จุด (modal · พิมพ์ HTML · build PDF) — defense in depth ใบที่ออกต้องตรงกับที่ UI บอก · **ถ้อยคำในใบต้องสอดคล้องกับเพดาน:** เขียน "มีรายได้เฉลี่ยเดือนละ X ซึ่งรวมเงินเดือนพื้นฐานและค่าตอบแทนอื่น ๆ แล้ว" (ของเดิม "ยังไม่รวมค่าตอบแทน" ขัดกับตัวเลขทันทีที่ยอมให้ระบุเกินพื้นฐาน) — มีเทสต์คุม |
...
| **`salaryDisabled` flag** | flag ใน employee doc → ปิดทั้งหมดของเงินเดือน (สำหรับฝึกงาน/Part-time/ลาออก) · slip ไม่ render รายการเลย · หน้าแรกไม่โชว์การ์ดเงินเดือน/ค่าคอม (`PositionRateCard`) |
| **ดูมุมมองพนักงาน (admin preview)** | admin เลือกพนักงาน → render หน้าตา**เหมือนที่พนักงานคนนั้นเห็นเป๊ะทุกอย่าง** (เบิก/ลา/เงินเดือน/การ์ด/สิทธิ์ ฯลฯ) · **กฎถาวร:** ทุกครั้งที่แก้/เพิ่ม UI ฝั่งพนักงาน ต้องเช็คว่า admin preview เห็นตรงกัน — data layer ฝั่ง admin บาง subscription scope ต่าง (เช่น advances = pending-only) ต้อง subscribe scoped ของพนักงานที่ preview เพิ่ม · vars `view*` (`viewEmployee`/`viewEmployeeId`/`viewSalaryDisabled`) override ตอน render ใน `App.tsx` |
...
- **Production (ใช้งานจริงแล้ว) — ต้อง backward compatible เสมอ:** มีข้อมูลจริง
  ...
  **ห้าม breaking changes ที่ทำข้อมูลเก่าพัง/อ่านไม่ออก/คำนวณเพี้ยน** เมื่อแก้ code:
  - **เปลี่ยน schema/field:** ต้อง migrate-on-read (รองรับทั้งของเก่า + ใหม่) ·
    เก็บ legacy field + fallback เสมอ (ดู pattern `resolvePoolItem*`/`resolveBonusItem*`/
    `ensureLegacy` ใน `salaryUtils.ts`) · ห้ามลบ/เปลี่ยนความหมาย field เดิมที่ data เก่าใช้
  - **เปลี่ยน logic คำนวณเงิน:** ต้องไม่ทำให้ยอดเดือนที่ freeze แล้วเปลี่ยน ·
    snapshot รายเดือน (roleId/เรท/totalLeaveDays ใน salary doc) คือ source of truth
    ของอดีต — อ่าน snapshot ก่อน live เสมอ
  - **rename/ลบ:** ต้อง propagate ไป denormalized copies (เช่น `employee.role`) หรือ
    ใช้ live lookup by id · ห้ามทิ้งข้อมูลค้าง
  - เพิ่ม/อัปเดตเทสต์ใน `src/utils/` ทุกครั้งที่แตะ logic เงิน (invariant: เงินไม่เพี้ยน +
    idempotency — ดู `docs/reference/testing.md`)

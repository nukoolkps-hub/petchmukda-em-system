...
| **หน้าที่ประจำ** (`duties` + `dutyAssignments`) | weekly/monthly rotation · `pickPrimary` เลือกคนทำตาม fairness (เคยทำน้อยสุดได้ก่อน) · **คนแทน (primary ลา):** monthly = fair "เคยแทนน้อยสุดก่อน" (`pickRotationSubstitute` + `replayRotationSubHistory`) · weekly = neighbor-scan · `duty.substituteExcludedEmpIds` = คนที่หมุนเป็นเวรหลักได้แต่ไม่ให้เป็นคนแทน · **`duty.exclusive`** ("ผูกขาดคนทำ" เช่น ONLINE/บัญชี) = คนที่ทำหน้าที่นี้วันนั้น (คนหลัก**หรือคนแทน**) ไม่ถูกจัดหน้าที่อื่นเลย — คำนวณหน้าที่ผูกขาดก่อนแล้วส่ง `blockedEmpIds` (hard filter · ไม่มี fallback) เข้าหน้าที่ที่เหลือ · `coveragePay` จ่ายแทนเมื่อ target ลา (โชว์สดใน preview ก่อนยืนยันยอดผ่าน snapshot `coverageThisMonth`) · **snapshot** `dutyAssignmentsToday` = assignments วันนี้ + `coverageForecast[]` (คนแทนล่วงหน้าถึงสิ้นปี) + `coverageThisMonth` (เงินค่าแทนสดต่อคน) · `applicableDuties` filter ด้วย storeCalendar ก่อน render · client/server sync ผ่าน `scripts/check-duty-sync.mjs` |

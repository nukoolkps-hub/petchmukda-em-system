...
| `src/utils/leaveUtils.ts` | นับวันลา, คำนวณ over-quota, `canCancelLeave` (เส้นตายยกเลิกใบลา 09:00 — single source คู่กับ `firestore.rules`) |
...
| เส้นตายพนักงาน "ยกเลิกใบลาเอง" | ก่อน **09:00 ของวันแรกที่ลา** (`LEAVE_CANCEL_CUTOFF_HOUR`) · หลังจากนั้น admin เท่านั้น · ล็อก 2 ชั้น (UI `canCancelLeave` + `firestore.rules` `canSelfCancelLeave` ใช้เวลาไทยฝั่ง server) |

...
- **ยกเลิกกลางคัน = stamp `cancelledAt` ไม่ลบ doc** — ประวัติต้องเห็นว่าเคยเริ่มแล้วเลิก · ยกเลิกแล้ว**ไม่ใช่ "หมดเวลา"** (`isExpired` คืน false) คนละเหตุผลกัน เหมารวมแล้ว UI อ่านผิด · `isInProgress` คือตัวที่บอกว่ายังทำอยู่จริง (ยังไม่ส่ง · ยังไม่ยกเลิก · ยังไม่หมดเวลา) · rules ปิดการเขียนของเจ้าของผ่าน `quizStillOpen()`

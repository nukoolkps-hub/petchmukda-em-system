...
export default function useFirebaseAppData(){
  ...
  const loading = empResult.loading || leavesResult.loading || salResult.loading ||
                  advResult.loading || rolesResult.loading || pcResult.loading;
  const error = empResult.error || leavesResult.error || salResult.error ||
                advResult.error || rolesResult.error || pcResult.error;
  ...
  const noop = () => console.warn("[Firebase mode] setter ไม่ถูกใช้ — เรียก action method แทน");
  ...
  return {
    ...
    updateEmployee, upsertEmployee,
    updateSalary,
    submitAdvance, updateAdvance, approveAdvance, rejectAdvance,

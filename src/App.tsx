...
import { bootstrapAdmin, type BootstrapAdminResult } from "./firebase/admin";
...
function UnlinkedEmployeeScreen({
  onSignOut,
  onBootstrapComplete,
}: {
  onSignOut: () => void;
  onBootstrapComplete: (result?: BootstrapAdminResult) => Promise<void>;
}) {
  ...
  async function handleBootstrapSubmit(event: FormEvent) {
    ...
    if (!secret) {
      setBootstrapError("กรุณากรอก Setup Secret");
      return;
    }
    ...
    setBootstrapError("");
    try {
      const result = await bootstrapAdmin(secret);
      await onBootstrapComplete(result);
    } catch (err: unknown) {
      console.error("[Bootstrap Admin] failed:", err);
      setBootstrapError((err as Error).message || "ตั้งค่าผู้ดูแลระบบไม่สำเร็จ");
    } finally {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center p-6 bg-cream font-sans">
      <div className="w-full max-w-[420px] px-5 py-6 bg-white rounded-[18px] border border-gold/25 text-center shadow-[0_8px_24px_rgba(90,30,10,0.10)]">
        ...
        <div className="mt-4 pt-4 border-t border-dashed border-bdr">
          {!showBootstrap ? (
            <button
              type="button"
              onClick={() => setShowBootstrap(true)}
              className="border-none bg-transparent text-xs font-semibold text-txt-soft underline underline-offset-4 cursor-pointer font-[inherit]"
            >
              ตั้งค่าผู้ดูแลระบบคนแรก
            </button>
          ) : (
            <form onSubmit={handleBootstrapSubmit} className="text-left">
              <label className="block text-xs font-semibold text-txt-soft mb-1.5">
                Setup Secret
              </label>
              <input
                type="password"
                ...
                className="w-full px-3 py-2.5 rounded-[10px] border border-bdr bg-cream text-sm text-txt outline-none font-[inherit] box-border"
                autoComplete="off"
              />
              {bootstrapError && (
                <div className="text-red text-xs mt-1.5">{bootstrapError}</div>
              )}
              <div className="flex gap-2 mt-3">
                <button
                  ...
                  className="flex-1 px-3 py-2 rounded-[9px] border border-bdr bg-white text-txt-mid text-sm font-semibold cursor-pointer font-[inherit]"
                ...
                <button
                  type="submit"
                  className="flex-1 px-3 py-2 rounded-[9px] border-none bg-maroon text-white text-sm font-bold cursor-pointer font-[inherit] disabled:opacity-60 disabled:cursor-wait"
                  disabled={bootstrapping}
                >
                  {bootstrapping ? "กำลังตั้งค่า..." : "ยืนยัน"}
                </button>
              </div>
            </form>
          )}
...
export default function LeaveApp() {
  ...
  async function refreshAdminClaim(bootstrapResult?: BootstrapAdminResult) {
    ...
    const nextIsAdmin = result.claims.admin === true;
    setIsAdmin(nextIsAdmin);
    setAdminChecked(true);
    if (!nextIsAdmin) {
      throw new Error("ยังไม่พบสิทธิ์ผู้ดูแลระบบหลังตั้งค่า");
    }
    navigate("/admin", { replace: true });
  }
  ...
  if (!currentEmployee && !isAdmin) {
    return (
      <UnlinkedEmployeeScreen
        onSignOut={authSignOut}
        onBootstrapComplete={refreshAdminClaim}
      />
    );
  }

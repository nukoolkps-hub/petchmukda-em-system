...
export default function LeaveApp() {
  ...
  useEffect(() => {
    if (profile) {
      ...
      if (emp && emp.role !== profile.role)
        setProfile((p) => ({ ...p, role: emp.role }));
    }
  }, [empDir, profile.role, profile.name, profile]);

  const myLeaves = allLeaves.filter(
    (lv) => profile && lv.empName === profile.name,
  );
  const balance = empDir.find((e) => profile && e.name === profile.name)
    ?.balance || { personal: 15, sick: 15 };
  const used = empDir.find((e) => profile && e.name === profile.name)?.used || {
    personal: 0,
    sick: 0,
  };

  const _selType = LEAVE_TYPES.find((t) => t.id === form.type);
  ...
  function handleDelete(id) {
    setAllLeaves((prev) => prev.filter((lv) => lv.id !== id));
    showToast("ลบรายการลาเรียบร้อยแล้ว");
  }
  function handleUpdateRole(empId, field, value) {
    setEmpDir((d) =>
      d.map((e) => (e.id === empId ? { ...e, [field]: value } : e)),
    );
  ...
  const pageTitle: Record<string, string | null> = {
    home: null,
    request: "ยื่นคำขอลา",
      ...
      <div className="leave-app-root">
        ...
        <div className="leave-main">
          ...
          <div className="leave-desktop-header relative overflow-hidden">
            ...
            >
              <defs>
                <linearGradient id="dh1" x1="0%" y1="0%" x2="100%" y2="100%">
                  <stop offset="0%" stopColor="#E8C87A" stopOpacity="0.22" />
                  <stop offset="100%" stopColor="#C9973A" stopOpacity="0.06" />
                </linearGradient>
                <linearGradient id="dh2" x1="100%" y1="0%" x2="0%" y2="100%">
                  <stop offset="0%" stopColor="#E8C87A" stopOpacity="0.28" />
                  <stop offset="100%" stopColor="#9B3030" stopOpacity="0.08" />
                </linearGradient>
              </defs>
              {[
                ["110,0 140,0 125,22", "dh2"],
                ["140,0 175,0 175,28 155,14", "dh1"],
                ["175,0 220,0 220,35 195,18", "dh2"],
                ["110,0 125,22 100,38 85,18", "dh1"],
                ["125,22 155,14 160,40 130,50", "dh2"],
                ["155,14 175,28 170,52 145,44", "dh1"],
                ["175,28 195,18 210,48 185,58", "dh2"],
                ["195,18 220,35 220,62 205,55", "dh1"],
              ].map(([p, g], i) => (
            ...
            >
              <defs>
                ...
                <linearGradient id="mg2" x1="100%" y1="0%" x2="0%" y2="100%">
                  <stop offset="0%" stopColor="#E8C87A" stopOpacity="0.30" />
                  <stop offset="100%" stopColor="#9B3030" stopOpacity="0.08" />
                </linearGradient>
                <linearGradient id="mg3" x1="0%" y1="100%" x2="100%" y2="0%">
            ...
            {tab === "home" && (
              <>
                ...
                {(() => {
                  ...
                  const _pct = Math.min(100, (usedThisMonth / quota) * 100);
                  return (
                    <div
                    ...
                    >
                      ...
                      <div className="flex gap-2.5 mb-3.5">
                        {Array.from({ length: quota }).map((_, i) => {
                          ...
                          const _overFill =
                            usedThisMonth > quota && i < usedThisMonth;
                          return (
                            <div
                              key={i}
                              style={{
            ...
            {tab === "request" && submitted && (
              <div className="text-center pt-10 pb-5">
                ...
                <div className="flex gap-2.5 justify-center flex-wrap">
                  <button
                    onClick={reset}
                    className="px-6 py-3.5 bg-gradient-to-br from-gold to-gold-lt text-maroon-dk border-none rounded-[14px] text-base font-bold cursor-pointer font-[inherit] shadow-[0_4px_14px_rgba(201,151,58,0.25)]"
                  >
                    + ยื่นคำขอใหม่
                  </button>
                  <button

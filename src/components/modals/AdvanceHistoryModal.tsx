...
export default function AdvanceHistoryModal({ advanceRequests, onClose }) {
  ...
  const sMap = {
    ...
    approved: {cls:"bg-green-lt text-green", label:"อนุมัติ • โอนแล้ว", icon:"✅"},
      ...
      <div className="bg-white rounded-t-3xl px-5.5 pt-6 pb-7 w-full max-w-[430px] shadow-[0_-12px_40px_rgba(45,26,14,0.25)] animate-[slideUp_0.3s_cubic-bezier(.22,.68,0,1.1)] max-h-[92vh] overflow-y-auto">
        ...
        <div className="flex items-center gap-3 mb-4.5">
          <div className="w-[46px] h-[46px] rounded-xl bg-gold-pale flex items-center justify-center border border-gold/25">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--color-maroon)" strokeWidth="2.2" strokeLinecap="round" strokeLinejoin="round">
              <circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/>
            </svg>
          </div>
          <div className="flex-1">
            <div className="font-extrabold text-lg text-txt">ประวัติการเบิก</div>
            <div className="text-xs text-txt-soft mt-0.5">คำขอเบิกเงินล่วงหน้าทั้งหมด</div>
          </div>
        </div>
            ...
            <div key={m} className="mb-4.5">
              ...
              <div className="flex flex-col gap-2">
                {monthList.map(r=>{
                  ...
                  return(
                    <div key={r.id} className="px-3.5 py-3 bg-cream rounded-xl border border-bdr">
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="text-lg font-extrabold text-txt">฿{TH_NUMBER(r.amount)}</div>
                        <span className={`text-[11px] font-semibold px-2.5 py-0.5 rounded-full whitespace-nowrap ${s.cls}`}>
                          {s.icon} {s.label}
                        </span>
                      </div>
                      <div className="text-xs text-txt-mid mb-1 leading-normal">{r.reason}</div>

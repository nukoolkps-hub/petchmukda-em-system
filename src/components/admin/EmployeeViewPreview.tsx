/* ─── EmployeeViewPreview — ปุ่ม/แบนเนอร์ "ดูมุมมองพนักงาน" (admin) ──────
...
export default function EmployeeViewPreview({
  ...
  return (
    <>
      {/* ปุ่มเข้าโหมด (เมื่อยังไม่ preview) */}
      {!current && (
        <button
          type="button"
          onClick={() => setPickerOpen(true)}
          className="fixed z-[1100] bottom-[80px] right-3 md:bottom-4 md:right-4 inline-flex items-center gap-1.5 px-3.5 py-2.5 rounded-full border-none bg-maroon text-white text-sm font-bold cursor-pointer font-[inherit] shadow-[0_6px_20px_rgba(123,28,28,0.37)] active:scale-[0.97] transition-transform duration-100"
        >
          <IconEye size={15} strokeWidth={2.5} />
          ดูมุมมองพนักงาน
        </button>
      )}
